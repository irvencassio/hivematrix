...
- **Model-advised decomposition** (`src/lib/intake/decompose.ts` + `src/lib/models/chat-client.ts`) — opt-in (`taskIntakeModelDecomposition` flag, default off). When a prompt is broad, a KEYLESS completion backend (local Qwen HTTP, else the codex/ChatGPT CLI session) proposes cleaner step text via `classifyIntakeAsync`; offline / no model / any failure → deterministic regex split. **No cloud LLM API keys, ever (no Anthropic, no OpenAI/ChatGPT key); Claude/Anthropic is never invoked.** The model only supplies step text — risk, the held release/deploy gate, dependency gating, and concurrency stay deterministic in `proposedItemsFromFragments`.
