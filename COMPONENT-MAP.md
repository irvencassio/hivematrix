...
- **Model-advised decomposition** (`src/lib/intake/decompose.ts` + `src/lib/models/chat-client.ts`) — automatic and LOCAL-ONLY: on whenever a keyless local loopback Qwen model (HTTP on 127.0.0.1) is configured, off otherwise (no setting). When a prompt is broad, the local model proposes cleaner step text via `classifyIntakeAsync`; offline / no local model / any failure → deterministic regex split. **No cloud LLM API keys, ever (no Anthropic, no OpenAI/ChatGPT key); Claude/Anthropic is never invoked and nothing leaves the machine.** The model only supplies step text — risk, the held release/deploy gate, dependency gating, and concurrency stay deterministic in `proposedItemsFromFragments`.
