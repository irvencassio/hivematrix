...
- **Agent loop** (`src/lib/flash/loop.ts`) — streams LM Studio (Qwen) via OpenAI-compatible SSE; executes lane tools + two flash-only tools (`persona_update`, `escalate_to_task` — creates a self-planning `workflow:"work"` task); budget: 12 tool calls / 3 min.
