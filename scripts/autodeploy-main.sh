...
echo
echo "This wrapper delegates the actual release work to scripts/developer-id-release.sh."
echo "scripts/developer-id-release.sh owns: version file updates, git add/commit/push,"
...
echo "  scripts/developer-id-release.sh       - canonical release lane (bump, build, sign, notarize, publish, verify)"
echo "  scripts/release-version.mjs           - version/build-number bump (monotonic)"
echo "  scripts/verify-provisioning-profile.mjs - HiveMatrix Core Developer ID profile gate"
echo "  scripts/notary-credentials.sh         - notary credential resolver (env or keychain profile)"
...
echo
echo "Running: bash scripts/developer-id-release.sh --release --marketing-version \"$VERSION\" --note \"$NOTE\""
bash scripts/developer-id-release.sh --release --marketing-version "$VERSION" --note "$NOTE"
