...
test("HiveMatrix release bundles Browser Lane and Terminal Lane artifacts as resources", () => {
  const tauri = read("src-tauri/tauri.conf.json");
  assert.match(tauri, /"\.\.\/build\/browser-lane\/Browser Lane\.app"\s*:\s*"lane-apps\/Browser Lane\.app"/);
  assert.match(tauri, /"\.\.\/build\/terminal-lane\/Terminal Lane\.app"\s*:\s*"lane-apps\/Terminal Lane\.app"/);
});

test("build-app packages and signs both standalone lane app artifacts before Tauri bundles resources", () => {
  const script = read("scripts/build-app.sh");
  assert.match(script, /node scripts\/package-browser-lane-app\.mjs/);
  assert.match(script, /node scripts\/package-terminal-lane-app\.mjs/);
  assert.match(script, /build\/browser-lane\/Browser Lane\.app/);
  assert.match(script, /build\/terminal-lane\/Terminal Lane\.app/);
