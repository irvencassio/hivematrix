...
export function buildResearchBriefMarkdown(input: ResearchBriefInput, context: ResearchBriefContext): string {
  ...
  return [
    ...
    `- Review and refine this brief, then use it to guide the content once it is approved.`,
    "",
...
const NEXT_ACTION = "Review and refine this brief, then use it to guide the content once approved.";
...
export async function prepareContentResearchBrief(input: ResearchBriefInput, deps: ResearchBriefDeps = {}): Promise<PrepareResearchBriefResult> {
  ...
  return {
    ...
    // The research brief is now a standalone deliverable — no downstream proposal.
    proposedAction: null,
  };
}
