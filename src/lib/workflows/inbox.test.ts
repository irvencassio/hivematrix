...
const SCRIPT = "content.video_script_from_brief";
...
test("a needs_review run appears in needs_review with a review nextAction", () => {
  const run = createWorkflowRun({ workflowId: SCRIPT, title: "Video script: AI tools", status: "needs_review" });
  const inbox = getWorkflowInbox();
...
test("an unapproved HeyGen action is blocked with review_required; approving moves it to ready", () => {
  const run = createWorkflowRun({ workflowId: SCRIPT, title: "script", status: "needs_review" });
  linkWorkflowRunArtifact(run.id, "scriptText", "the script");
  const a = proposeWorkflowAction({ sourceRunId: run.id, targetWorkflowId: HEYGEN, title: "Video", suggestedInputs: { title: "Video", script: "the script" } });
...
test("a missing-input action is blocked with the exact fields", () => {
  const run = createWorkflowRun({ workflowId: HEYGEN, title: "done run", status: "done" });
  const a = proposeWorkflowAction({ sourceRunId: run.id, targetWorkflowId: HEYGEN, title: "Video", suggestedInputs: { title: "Video" } });
  const inbox = getWorkflowInbox();
  const item = inbox.groups.proposed_actions_blocked.find((i) => i.id === a.id);
  assert.ok(item);
  assert.equal(item.status, "needs_input");
  assert.match(item.blockedReason ?? "", /script/);
});

test("rejected, changes_requested, failed runs land in the attention/changes groups", () => {
  const r1 = createWorkflowRun({ workflowId: SCRIPT, title: "rej", status: "needs_review" });
  reviewWorkflowRun(r1.id, "reject", {});
  const r2 = createWorkflowRun({ workflowId: SCRIPT, title: "chg", status: "needs_review" });
  reviewWorkflowRun(r2.id, "request_changes", {});
  createWorkflowRun({ workflowId: HEYGEN, title: "fail", status: "failed" });
  createWorkflowRun({ workflowId: HEYGEN, title: "ok", status: "done" });
...
test("the inbox leaks no secret-looking artifact keys or values", () => {
  const run = createWorkflowRun({ workflowId: SCRIPT, title: "s", status: "needs_review" });
  linkWorkflowRunArtifact(run.id, "scriptText", "narration with token=SHOULD_NOT_APPEAR");
  proposeWorkflowAction({ sourceRunId: run.id, targetWorkflowId: HEYGEN, title: "Video", suggestedInputs: { title: "Video", script: "x", sessionCookie: "LEAK" } });
...
test("formatWorkflowInboxSummary is concise and operational", () => {
  createWorkflowRun({ workflowId: SCRIPT, title: "a", status: "needs_review" });
