...
const WF = "content.research_brief";
...
test("createWorkflowRun validates the workflowId against the registry", () => {
  ...
  const run = createWorkflowRun({ workflowId: WF, title: "Launch", draftId: "d1" });
  assert.equal(run.workflowId, WF);
  assert.equal(run.lane, "review");             // defaulted from the registry def
  assert.equal(run.capability, "research.brief");
  assert.equal(run.runbook, "docs/runbooks/content-research-brief.md");
...
test("artifacts + links merge and events redact secret-looking metadata", () => {
  ...
  appendWorkflowRunEvent(run.id, "note", "review session", { traceRunId: "t1", sessionCookie: "leak-me", password: "leak-me-too" });
...
test("findWorkflowRunByDraft returns the latest run for a draft", () => {
  createWorkflowRun({ workflowId: WF, title: "first", draftId: "d1" });
  const second = createWorkflowRun({ workflowId: WF, title: "second", draftId: "d1" });
...
test("a needs_review run is review-blocked until approved", () => {
  const run = createWorkflowRun({ workflowId: WF, title: "r", status: "needs_review" });
...
test("request_changes and reject keep the run blocked, and the note is secret-scrubbed", () => {
  const a = createWorkflowRun({ workflowId: WF, title: "a", status: "needs_review" });
  ...
  const b = createWorkflowRun({ workflowId: WF, title: "b", status: "needs_review" });
